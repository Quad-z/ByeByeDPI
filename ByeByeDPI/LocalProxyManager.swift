...
final class LocalProxyManager: ObservableObject {
    ...
    func start() {
        do {
            ...
            listener.service = .init(name: "ByeByeDPI Proxy", type: "_http._tcp")
    ...
    private func handleConnectTunnel(_ request: String, _ data: Data, _ connection: NWConnection) {
        guard let targetInfo = extractTarget(from: request) else {
            ...
            connection.send(content: badRequest, completion: NWConnection.SendCompletion.contentProcessed({ _ in connection.cancel() }))
            return
        }

        let target = NWConnection(host: NWEndpoint.Host(targetInfo.host), port: NWEndpoint.Port(rawValue: targetInfo.port) ?? 443, using: .tcp)
        target.start(queue: DispatchQueue.global(qos: .utility))

        target.stateUpdateHandler = { [weak connection] (state: NWConnection.State) in
            guard case .ready = state, let connection else { return }
            let established = "HTTP/1.1 200 Connection Established\r\n\r\n".data(using: .utf8) ?? Data()
            connection.send(content: established, completion: NWConnection.SendCompletion.contentProcessed({ _ in
    ...
    private func handleHttpProxy(requestData data: Data, connection: NWConnection) {
        ...
        let defaultPort: UInt16 = url.scheme == "https" ? 443 : 80
        let port = NWEndpoint.Port(rawValue: UInt16(url.port ?? Int(defaultPort))) ?? NWEndpoint.Port(rawValue: defaultPort)!
        let target = NWConnection(host: NWEndpoint.Host(url.host ?? ""), port: port, using: .tcp)
        target.start(queue: DispatchQueue.global(qos: .utility))

        target.stateUpdateHandler = { (state: NWConnection.State) in
            guard case .ready = state else { return }
            target.send(content: data, completion: NWConnection.SendCompletion.contentProcessed({ _ in
                pipe(connection, target)
            }))
        }
    }
